...
아래에서 `YoY 10%p 하락`은 YoY 값 자체의 차이를 뜻합니다. 예를 들어 YoY 정점이
`50%`라면 현재 `40%`가 10%p 하락이고, `35%`가 15%p 하락입니다. `50% → 45%`를
상대하락 10%로 계산하지 않습니다.

### 1.1 미국주식 매수 기준

#### A. 최근 48개월 안에 FINRA YoY +50% 이상 이력이 없는 평시·횡보장

- 현재 YoY `-10% 이하`: **약한 매수**
- 현재 YoY `-15% 이하`: **강한 매수**

#### B. 최근 48개월 안에 FINRA YoY +50% 이상 이력이 있는 과열 후 국면

- 현재 YoY `-25% 이하`: **약한 매수**
- 현재 YoY `-30% 이하`: **강한 매수**
- A와 B는 서로 배타적입니다. +50% 이상 이력이 있으면 평시·횡보장 기준을 쓰지 않습니다.
- 절대 마진잔고 고점 대비 `-20%/-25%/-30%`와 최근 3개월 하락 둔화는 추가 정보이며,
  핵심 YoY 매수조건과 AND로 묶지 않습니다.

#### 미국 종목별 매수 보조지표

| 종목 | 200일선 하방 이격률 |
|---|---:|
| Alphabet, Amazon | `-25% 이하` |
| NVIDIA, Broadcom, TSMC, ASML, Lam Research, KLA | `-30% 이하` |

이 이동평균 조건은 시장 매수신호를 차단하는 필수조건이 아니라 종목별 보조정보입니다.

### 1.2 미국주식 매도 기준

전제는 최근 48개월 FINRA YoY 정점이 `+50% 이상`인 것입니다.

- 현재 YoY가 정점보다 `10%p 이상` 낮아짐: **약한 매도**
- 현재 YoY가 정점보다 `15%p 이상` 낮아짐: **강한 매도 조건 활성화**

강한 조건 이후 목표월은 다음과 같이 계산합니다.

1. 강한 조건을 처음 달성한 당시 YoY가 `+50% 이하`이면 `YoY 정점월 +3개월`
2. 강한 조건을 처음 달성한 당시 YoY가 아직 `+50% 초과`이면 당장은 목표월을 잡지 않고
   기다린 뒤, `최초 +50% 이하 월 +3개월`
3. 목표월 전에는 **강한 매도 / 3개월 후 매도**, 목표월에는
   **강한 매도 / 당월 매도 필요**로 표시
4. 같은 예약상태가 유지되는 중간 달에는 반복 알림을 보내지 않음

#### 미국 종목별 매도 보조지표

| 종목 | 약한 과열 | 강한 과열 |
|---|---:|---:|
| Alphabet, Amazon | 120일선 `+25% 이상` | 120일선 `+30% 이상` |
| NVIDIA, Broadcom, TSMC, ASML, Lam Research, KLA | 120일선 `+35% 이상` | 120일선 `+40% 이상` |

이 이동평균 조건도 FINRA 시장 매도신호의 필수조건이 아니라 종목별 보조정보입니다.

### 1.3 한국주식 매수 기준

#### 시장 신호

- 최근 48개월 전체 신용잔고 YoY 정점 `+60% 이상`: 선행 과열 전제
- 현재 YoY `-25% 이하`: **약한 매수**
- 현재 YoY `-30% 이하`: **강한 매수**

#### 실제 종목별 강한 매수 준비조건

강한 시장신호와 함께 아래 세 묶음을 모두 확인합니다.

1. 전체 신용잔고 절대액이 고점 대비 `-30% 이하`
2. 해당 종목의 200일선 하방 이격률 충족
3. 해당 종목의 `개별 신용잔고 하락폭` 또는 `신용잔고율 최근 5년 하위 20%` 중 하나 충족

| 종목 | 200일선 | 개별 신용잔고 하락폭 | 대체조건 |
|---|---:|---:|---:|
| 삼성전자 | `-25% 이하` | 고점 대비 `-30% 이하` | 잔고율 하위 20% |
| SK하이닉스 | `-30% 이하` | 고점 대비 `-35% 이하` | 잔고율 하위 20% |

### 1.4 한국주식 매도 기준

#### 시장 매도조건: 모두 충족

1. 최근 48개월 전체 신용잔고 YoY 정점 `+60% 이상`
2. 현재 YoY가 그 정점보다 `15%p 이상` 하락
3. 정점 이후 전체 신용잔고 절대액이 전월 대비 처음 감소한 이력 존재
4. 최근 18개월 안에 전체 신용잔고 절대액이 당시 고점 대비 `-45% 이하`로 붕괴한 이력 없음

#### 종목별 매도 보조조건

| 종목 | 120일선 약한 과열 | 120일선 강한 과열 | 대체조건 |
|---|---:|---:|---:|
| 삼성전자 | `+25% 이상` | `+30% 이상` | 신용잔고율 최근 5년 상위 10% |
| SK하이닉스 | `+35% 이상` | `+40% 이상` | 신용잔고율 최근 5년 상위 10% |

- 시장 매도조건만 충족: **약한 매도**
- 시장 매도조건과 종목별 `120일선 약한 과열 OR 잔고율 상위 10%`를 함께 충족:
  **강한 매도**
- 한국주식에는 미국주식처럼 신호 후 3개월을 기다리는 규칙이 없습니다.

### 1.5 BTC 매수 기준

#### 약한 매수: 모두 충족

1. 현재 가격 기준 전고점 후 `9~15개월`
2. 전고점 대비 가격 `-60% 이하`
...
1. 전고점 후 `9~15개월`
2. 전고점 대비 가격 `-70% 이하`
3. 아래 파생조건 6개 중 `2개 이상`

- OI 고점 대비 `-50% 이하`
- 강한 OI 청산: OI 고점 대비 `-60% 이하`
- 7일 평균 펀딩비 `0 이하`
- 30일 평균 펀딩비 `0 이하`
- 7일 가격 `-10% 이하`이면서 OI `-20% 이하`인 대규모 롱 청산 프록시
- OI 21일 변화율 절댓값 `2% 이하`
...
1. 설정된 직전 확정 대형고점에서 `45개월 이상` 경과
   - 42~45개월: 예비 관찰
   - 45~54개월: 핵심 고점창
   - 54개월 초과: 지연 상태로 계속 감시
2. 현재 120일선 상방 이격률이 최근 3년 분포의 `상위 10%`
3. 아래 파생 과열 3개 중 `2개 이상`
   - OI 최근 3년 상위 10%
   - 펀딩비 최근 3년 상위 10%
   - 선물 베이시스 최근 3년 상위 10%
...
약한 매도조건에 더해 아래 꺾임 중 `1개 이상`을 확인합니다.

- OI 최근 180일 고점 대비 `-10% 이하`
- 7일 펀딩비 최근 180일 고점 대비 `-30% 이하`
- 베이시스 최근 180일 고점 대비 `-25% 이하`
- 가격이 사이클 고점 대비 `-12% 이하`이고 OI 또는 펀딩비도 감소

OI·펀딩비·베이시스 꺾임은 가격이 신고가 대비 10% 이내일 때만 인정합니다. BTC에는
강한 매도 후 추가 3개월 대기규칙이 없습니다.
...
- 7일 평균 펀딩비 음수
- 7일 가격 `-10% 이하`이면서 OI `-20% 이하`인 대규모 롱 청산 프록시
- SOL/BTC 최근 30일 하락세가 직전 30일보다 둔화
- OI 21일 변화율 절댓값 `2% 이하`
...
1. BTC와 SOL 각각의 설정된 직전 확정 대형고점에서 모두 `42개월 이상` 경과
   - 42~54개월: 핵심 고점창
   - 54개월 초과: 지연 상태로 계속 감시
2. `사이클 저점 대비 10배 이상` 또는 현재 120일선 상방 이격률이 최근 3년 상위 10%
3. 아래 파생 과열 4개 중 `2개 이상`
   - SOL OI 최근 3년 상위 10%
   - SOL OI 180일 증가율 `+100% 이상`
   - 7일·30일 펀딩비 중 높은 분위가 최근 3년 상위 10%
   - SOL 선물 베이시스 최근 3년 상위 10%

#### 강한 매도

약한 매도조건에 더해 아래 꺾임 중 `1개 이상`을 확인합니다.

- OI 최근 180일 고점 대비 `-15% 이하`
- 7일 펀딩비 최근 180일 고점 대비 `-30% 이하`
- SOL/BTC 사이클 고점 대비 `-10% 이하`
- SOL 가격 사이클 고점 대비 `-15% 이하`

SOL에도 강한 매도 후 추가 3개월 대기규칙이 없습니다.
...
`config.yml`의 기존 상대하락 미국·한국 매도 설정은 각각 `10/15%p`, `15%p`
하락폭 설정으로 자동 이전합니다.
